<p align="center">
  <img src="screens/itbee-logo.png" alt="ITBee Logo" width="200"/>
</p>

<h1 align="center">🐝 ITBee</h1>

<p align="center">
  <strong>AI-Powered Network Discovery & Management</strong><br/>
  <em>"Buzz Buzz! You're the Queen. The bees will handle the rest."</em>
</p>
...
In a beehive, the Queen doesn't carry pollen. She doesn't build honeycomb. She doesn't guard the entrance. **The bees do all the heavy lifting.**

ITBee works the same way. **You are the Queen.** Your network is the hive. And the bees — our AI-powered agents — will discover, manage, configure, and protect every device in your infrastructure. You just tell them what you need.
...
<p align="center">
  <img src="screens/graph.png" alt="ITBee Network Graph" width="800"/>
</p>
...
Getting started with ITBee is as simple as adding your first device:
...
That's it. The device appears in your hive graph, and ITBee automatically discovers every other device it can see — gateways, DNS servers, DHCP servers, ARP neighbors — all added to the graph as unmanaged devices. Toggle them on or off with the "Unmanaged" switch.
...
When you add a device, ITBee deploys **scout bees** — lightweight commands that map the device and everything around it:
...
From this data, ITBee builds your hive:
- **Managed devices** (devices you scanned) — shown prominently
- **Gateways** — detected from routing tables, shown as routers
- **DNS & DHCP servers** — detected from network config
- **ARP neighbors** — every device seen on the wire

### AI Console — The Worker Bees

Once a device is in the hive, the AI Console is your command center. The worker bees can:

- **Run remote commands** on the device (SSH / WinRM)
- **Fetch files** from the device for local analysis
- **Execute local Python scripts** for parsing binary files, crunching data, or processing logs
- **Chain actions** across multiple iterations — fetch a file, parse it, run more commands, and synthesize a final answer

The AI maintains **conversation history per device**, so it remembers what you discussed earlier. Ask a follow-up — it knows the context.

### Cross-Device Intelligence — The Hive Mind

ITBee maintains a **global context** of your entire network. This means the AI understands relationships between devices — which machine is the router, which is the DNS server, which is the domain controller. This enables **complex multi-device operations from a single prompt**.
...
$ # Downloading Chrome MSI and configuring software installation policy...
...
ITBee works with **any Windows or Unix-like machine**. If it has a network interface, ITBee can discover it. If it supports SSH or WinRM, ITBee can manage it.
...
**The sky is the limit.** If you can SSH or WinRM into it, ITBee can manage it.
...
- 📦 **Agent Installation from Web Console** — install the ITBee agent directly from the UI onto any machine
- 🔄 **Continuous Monitoring** — bees that keep watching for changes in your hive
- 🛡️ **Security Scanning** — automated vulnerability discovery across the hive
...
Click **Add Device**, enter the IP, username, password, and select WinRM or SSH. ITBee connects immediately and scans.
...
<p align="center">
  <strong>🐝 ITBee</strong><br/>
  <em>Your bees. Your hive. Your network.</em><br/><br/>
  Stop doing IT. Let ITBee do IT for you.
</p>
